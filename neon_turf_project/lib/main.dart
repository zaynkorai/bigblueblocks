...
class _GameScreenState extends State<GameScreen> {
  // Reduced grid size makes blocks larger
  int gridSize = 10; 
  List<List<int>> grid = []; 
  int capturedCount = 0;
  ...
  bool _hasSwipedInCurrentGesture = false;

  void _handlePanStart(DragStartDetails details) {
    if (gameState != 'PLAY') return;
    _panAccumulator = Offset.zero;
    _hasSwipedInCurrentGesture = false;
  }

  void _handlePanUpdate(DragUpdateDetails details) {
    if (gameState != 'PLAY' || _hasSwipedInCurrentGesture) return;
    
    _panAccumulator += details.delta;
    double threshold = 20.0; // swipe pixels required to trigger move
    
    if (_panAccumulator.dx.abs() > threshold || _panAccumulator.dy.abs() > threshold) {
      int dx = 0;
      int dy = 0;
      
      // Determine dominant axis to ensure one-sided (no diagonal) moves
      if (_panAccumulator.dx.abs() > _panAccumulator.dy.abs()) {
        dx = _panAccumulator.dx > 0 ? 1 : -1;
      } else {
        dy = _panAccumulator.dy > 0 ? 1 : -1;
      }
      ...
      setState(() {
        ...
        _hasSwipedInCurrentGesture = true; // Lock until finger is lifted
        _refreshPreview();
      });
    }
  }
